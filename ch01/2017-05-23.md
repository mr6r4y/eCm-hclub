...
```
The story of memory offsets in assembly and disassemly for x86 is not complete. In practice however and at the current
level of our understanding it is more than enough to know. Later we will come accross code for x86 that runs 
in processor's real mode and the memory model is not `flat` - there are segment registers added to the pointer arithmetic.

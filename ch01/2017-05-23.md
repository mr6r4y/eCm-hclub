...
```
which means: "Use what is between `[` and `]` as address value and take `byte`/`word`/`dword`/`qword` from that memory address".
There several flavors of this bracket syntax which mean the same thing. In some cases instead of `byte`/`word`/`dword`/`qword`
a `offset` keyword is found. The meaning is the same. The lengths of `byte`/`word`/`dword`/`qword` are:

...
use `mov` to do memory-to-memory move and you have to use a register instead:
...
Some instruction allow using the `rep`(REPeat) prfeix.

##### [movs[b/w/d/q]](http://www.felixcloutier.com/x86/MOVS:MOVSB:MOVSW:MOVSD:MOVSQ.html)

These instructions are used to move data memory-to-memroy. The source memory address is
pointed by `rsi`/`esi` register and the destination is pointed by `rdi`/`edi` register.
This means the two registers must be initialiased before using `movs[b/w/d/q]`:

```nasm
; ...
mov ecx, 6            ; Set `ecx` to be the counter for the `rep` prefix mode
mov esi, 0x10090f58   ; Set source for `movsd`
mov edi, 0x10090f60   ; Set destination for `movsd`
; ...
; `movs[b/w/d/q]` instructions are single byte op code with no operands.
; You can see these with added non-existing implicit operands of destination
; and source for convenience of the disassembly reading.
;
